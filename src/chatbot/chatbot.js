...
const Chatbot = () => {
  ...
  const handleSend = async () => {
    ...
    const userMessage = { type: 'user', text: `Talk in ${selectedLanguage} ${textInput}` };
    conversation.push(userMessage);

    const botResponse = await generateAIResponse(conversation);
    if (botResponse !== null) {
      const updatedMessages = [...conversation, { type: 'bot', text: botResponse }];
  ...
  const renderChatItem = (item, index) => {
    ...
    const displayText = item.type === 'user' ? item.text.replace(languagePrefix, '') : item.text;

    return (
      <div key={index} className={item.type === 'user' ? 'userChat' : 'botChat'}>
